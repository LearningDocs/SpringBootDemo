...
public class Fetcher {
    ...
    public <T extends DefaultAbstractCrawlJob> Fetcher(int maxDepth, ObjectFactory<T> jobFactory) {
        ...
        SimplePool simplePool = new SimplePool<>(ConfigProperites.getFetchQueueSize(), jobFactory);
    ...
    public <T extends DefaultAbstractCrawlJob> void start() throws Exception {
        ...
        while (!fetchQueue.isOver) {
            ...
            if (crawlMeta == null) {
                Thread.sleep(ConfigProperites.getEmptyQueueWaitTime());
                continue;
            }


            try {
                long sleep = ConfigProperites.getSleepTime();
                Thread.sleep(sleep);
